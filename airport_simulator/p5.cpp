...
p5::p5()
{
    ...
    third_runway = new Runway(queue_limit);
}

p5::p5(int endTime, int queueLimit, double arrivalRate, double departureRate)
    : p1{ endTime, queueLimit, arrivalRate, departureRate }
{
    arrival_runway = new Runway(queue_limit);
    departure_runway = new Runway(queue_limit);
    third_runway = new Runway(queue_limit);
...
void p5::initialize()
{
    cout <<"*******************************************************************************"<<endl
        ...
        << "Third runway is for landings unless queue is empty" << endl
        <<"*******************************************************************************" << endl
...
void p5::run()
{
    ...
    {                                                       //  loop over time intervals
        ...
        for (int i = 0; i < number_arrivals; i++)
        {
            ...
            {
                if (third_runway->can_land(current_plane) != success)
                {
        ...
        for (int j = 0; j < number_departures; j++)
        {
            ...
            if (departure_runway->can_depart(current_plane) != success) {
                if (third_runway->can_depart(current_plane) != success) {
            ...
            switch (third_runway->activity_3(current_time, moving_plane))
            {
                //  Let at most one Plane onto the Runway at current_time.
            case Runway_activity::land:
                moving_plane.land(current_time);
            ...
            case Runway_activity::idle:
                ...
                Runway_activity activity1 = arrival_runway->movePlaneToOtherRunway(current_time, moving_plane);
                ...
                else if (activity1 != Runway_activity::idle) {
                ...
                {
                    third_runway->idle();
                    run_idle(current_time, "Third_runway");
...
void p5::run_idle(int time, string runway)
{
...
void p5::shut_down()
{
    ...
    cout << "*********************************\n";
    cout << "Stats for third landing Runway\n";
    cout << "*********************************\n";

    third_runway->shut_down(end_time);
